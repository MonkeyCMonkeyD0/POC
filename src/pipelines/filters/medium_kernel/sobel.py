...
class SobelFilter(nn.Module):
    def __init__(self):
        super().__init__()
        self.filter_x = nn.Conv2d(in_channels=3, out_channels=1, groups=1, kernel_size=5, stride=1, padding='valid', bias=False)
        self.filter_y = nn.Conv2d(in_channels=3, out_channels=1, groups=1, kernel_size=5, stride=1, padding='valid', bias=False)
        ...
        kernel_x = torch.tensor([
            ...
            [4, 2, 0, -2, -4],
            [2, 1, 0, -1, -2],
            [2, 1, 0, -1, -2]], dtype=torch.float)
        kernel_y = torch.tensor([
            [2, 2, 4, 2, 2],
            [1, 1, 2, 1, 1],
            [0, 0, 0, 0, 0],
            [-1, -1, -2, -1, -1],
            [-2, -2, -4, -2, -2]], dtype=torch.float)
